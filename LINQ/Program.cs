...
namespace CS3260_Lab12_KH
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Query 0 – find values in odd divisible by 3
            IEnumerable<int> q0 =
                from num in odds
                where num % 3 == 0
                select num;
            ...
            //Print query 0
            Console.Write("Query 0\n Values in Odd divisible by three: ");
            foreach (int i in q0)
                Console.Write(i + ", ");
            Console.WriteLine("\n");

            //Query 1 – Find values in data where 500 > id > 100 &&
            //  (name contains y or job contains e or job == student or programmer)
            IEnumerable<Object> q1 =
                from item in data
                where item.ID > 100
                where item.ID < 500
                where (item.Name.Contains('y') || item.Job.Contains('e') || item.Job.Contains('E')
                    || item.Job == "Student" || item.Job == "Programmer")
                select item;

            //Print query 1
            Console.WriteLine("Query 1\nPeople with IDs between 101 and 499 who are programmers, students,\nor any profession with an e, or whose name contains a y");
            foreach (var v in q1)
                Console.Write(v + "\n");
            Console.WriteLine("\n");


            //Query 2 – Find values in productList where 10 > productID > 66
            //in descending order by UnitsInStock, and write ProductName and UnitsInStock
            var q2 =
                from product in GetProductList()
                where product.ProductID < 10
                where product.ProductID < 66
                orderby product.UnitsInStock descending
                select product;

            //Print query 2
            Console.WriteLine("Query 2");
            Console.WriteLine(String.Format("{0,-32} {1,-5}", "Product", "Stock"));
            foreach (var v in q2)
                Console.WriteLine(String.Format("{0,-32} {1,-2}", v.ProductName, v.UnitsInStock));
            Console.WriteLine("\n");

            //Query 3 – Just go crazy on the productList
            var q3 =
                from product in GetProductList()
            ...
            foreach (var v in q3)
            {
                Console.WriteLine(v.Group + "\'s most expensive item price where the ID is an even number: $" + v.MostExpensive.ToString("F2"));
            }
            Console.WriteLine("\n");
